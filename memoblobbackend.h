...
public:
    ...
    // Phase 5: declare (note, palm) and present raw PalmRecord wire bytes. The
    // palm<->markdown conversion now lives in PalmToMarkdownStage/
    // MarkdownToPalmStage (registered via NoteDomainExtension); the engine sees
    // the full palm->markdown->canon chain and the on-disk peer is a
    // MarkdownFilesBackend declaring (note, markdown).
